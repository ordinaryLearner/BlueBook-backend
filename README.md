...
| `likes` | array | 点赞该评论的用户列表（User 对象数组，元素含 `id` / `username` / `account` / `avatar`；可为空） |
...
无需登录（浏览他人主页时可调用）。客户端把要查看的用户的 `userId` 放在路径中，服务端返回该用户**发布的所有帖子**，按发布时间倒序排列，结构与 `GET /api/posts` / `/my` 的元素一致（`data` 为完整帖子对象数组，可能为空数组，含 `sender` / `medias` / `likes` / `favourite` / `comments`）。用户不存在或没发过帖子时 `data` 返回空数组 `[]`。

**Response `200`：** `data` 为该用户的帖子对象数组（结构与 `/my` 相同）。
